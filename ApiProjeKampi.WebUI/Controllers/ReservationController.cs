...
namespace ApiProjeKampi.WebUI.Controllers
{
    public class ReservationController : Controller
    {
        ...
        public async Task<IActionResult> CreateReservation(CreateReservationDto createReservationDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7245/api/Reservations", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7245/api/Reservations?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("ReservationList");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7245/api/Reservations/GetReservation?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateReservation(UpdateReservationDto updateReservationDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7245/api/Reservations", stringContent);
            return RedirectToAction("ReservationList");
